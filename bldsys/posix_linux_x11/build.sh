...
OUT_BINS=("libpt_math.so" "libpt_mcrt.so" "libpt_tbbmalloc.so" "libpt_irml.so" "libpt_tbb.so" "libpt_gfx.so" "pt_launcher" "pt_gfx_malloc_test" "pt_general_acyclic_graphs_of_tasks")
