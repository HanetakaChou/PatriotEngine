...
)
{
	...
	ID3D12Heap *pHeapBuffer;
	{
		...
		Desc.Properties.Type = D3D12_HEAP_TYPE_CUSTOM; //Upload和Readback会限制InitialState，从而限制对Map/Unmap的优化 //即使在语义上为Upload和Readback的堆，也应当为Custom
...
static PT3DInstanceImpl *s_3DInstanceD3D12_Singleton_Pointer = reinterpret_cast<PT3DInstanceImpl *>(uintptr_t(0X1U));//不可能为有效值，因为不满足Alignment
