 ...
 */
#if defined(RTE_ARM64_MEMCPY_ALIGNED_THRESHOLD) || defined(RTE_ARM64_MEMCPY_UNALIGNED_THRESHOLD)
...
static __rte_always_inline void rte_mov64(uint8_t *dst, const uint8_t *src)
{
    ...
    const __uint128_t
        x0 = src128[0],
        x1 = src128[1], x2 = src128[2], x3 = src128[3];
    dst128[0] = x0;
    dst128[1] = x1;
    dst128[2] = x2;
    dst128[3] = x3;
...
static __rte_always_inline void rte_memcpy_ge128(uint8_t *dst, const uint8_t *src, size_t n)
{
    do
    {
        rte_mov128(dst, src);
...
static __rte_always_inline void rte_memcpy_ge16_lt64(uint8_t *dst, const uint8_t *src, size_t n)
{
    if (n == 16)
    {
        rte_mov16(dst, src);
...
static __rte_always_inline void rte_memcpy_ge64(uint8_t *dst, const uint8_t *src, size_t n)
{
    do
    {
        rte_mov64(dst, src);
