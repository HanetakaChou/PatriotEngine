    ...
    pt_output_stream_destroy_callback cache_output_stream_destroy_callback)
{
    ...
    // Frame
    if (!this->init_frame(cache_input_stream_init_callback, cache_input_stream_stat_size_callback, cache_input_stream_read_callback, cache_input_stream_destroy_callback, cache_output_stream_init_callback, cache_output_stream_write_callback, cache_output_stream_destroy_callback))
    {
        return false;
    }
    ...
    pt_output_stream_destroy_callback cache_output_stream_destroy_callback)
{
...
#if defined(PT_GFX_DEBUG_MCRT) && PT_GFX_DEBUG_MCRT
    mcrt_assert_spin_init(&this->m_asset_spinlock_wsi_window_exist);
#endif
    ...
    this->m_pipeline_lighting_mesh = VK_NULL_HANDLE;
    this->m_pipeline_cache_lighting_mesh = VK_NULL_HANDLE;
...
inline bool gfx_connection_vk::update_framebuffer()
{
    ...
    {
        PT_MAYBE_UNUSED bool res_load_pipeline_cache = this->load_pipeline_cache(".cache-vkpipeline-mesh.bin", &this->m_pipeline_cache_lighting_mesh);
        assert(res_load_pipeline_cache);
    }

    // pipeline // depend on renderpass
    assert(VK_NULL_HANDLE == this->m_pipeline_lighting_mesh);
    {
        ...
        stages[0].module = this->m_shader_module_lighting_mesh_vertex;
        stages[0].pName = "main";
        ...
        stages[1].module = this->m_shader_module_lighting_mesh_fragment;
        stages[1].pName = "main";
        ...
        PT_MAYBE_UNUSED VkResult res_create_graphics_pipelines = this->m_device.create_graphics_pipelines(this->m_pipeline_cache_lighting_mesh, 1U, graphics_pipeline_create_infos, pipelines);
        assert(VK_SUCCESS == res_create_graphics_pipelines);

        this->m_pipeline_lighting_mesh = pipelines[0];
    }
...
inline void gfx_connection_vk::destory_framebuffer()
{
    ...
    if (0U != this->m_swapchain_image_count)
    {
        ...
        this->store_pipeline_cache(".cache-vkpipeline-mesh.bin", &this->m_pipeline_cache_lighting_mesh);

        assert(VK_NULL_HANDLE != this->m_pipeline_lighting_mesh);
        this->m_device.destroy_pipeline(this->m_pipeline_lighting_mesh);
        this->m_pipeline_lighting_mesh = VK_NULL_HANDLE;
...
#include "imaging/vk/forward_shading_mesh_vertex.inl"
        ...
        shader_module_create_info.codeSize = sizeof(shader_code_lighting_mesh_vertex);
        shader_module_create_info.pCode = shader_code_lighting_mesh_vertex;

        PT_MAYBE_UNUSED VkResult res_create_shader_module = this->m_device.create_shader_module(&shader_module_create_info, &this->m_shader_module_lighting_mesh_vertex);
...
#include "imaging/vk/forward_shading_mesh_fragment.inl"
        ...
        shader_module_create_info.pNext = NULL;
        shader_module_create_info.flags = 0;
        shader_module_create_info.codeSize = sizeof(shader_code_lighting_mesh_fragment);
        shader_module_create_info.pCode = shader_code_lighting_mesh_fragment;

        PT_MAYBE_UNUSED VkResult res_create_shader_module = this->m_device.create_shader_module(&shader_module_create_info, &this->m_shader_module_lighting_mesh_fragment);
        assert(VK_SUCCESS == res_create_shader_module);
    }
...
mcrt_task_ref gfx_connection_vk::opaque_subpass_task_execute(mcrt_task_ref self)
{
    ...
    // bind - each shader
    gfx_connection->m_device.cmd_bind_pipeline(secondary_command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, gfx_connection->m_pipeline_lighting_mesh);
...
{
    assert(VK_NULL_HANDLE != this->m_shader_module_lighting_mesh_vertex);
    this->m_device.destroy_shader_module(this->m_shader_module_lighting_mesh_vertex);

    assert(VK_NULL_HANDLE != this->m_shader_module_lighting_mesh_fragment);
    this->m_device.destroy_shader_module(this->m_shader_module_lighting_mesh_fragment);
}
...
#if defined(PT_GFX_DEBUG_MCRT) && PT_GFX_DEBUG_MCRT
    mcrt_assert_spin_lock(&this->m_asset_spinlock_wsi_window_exist);
#endif
...
#if defined(PT_GFX_DEBUG_MCRT) && PT_GFX_DEBUG_MCRT
    mcrt_assert_spin_unlock(&this->m_asset_spinlock_wsi_window_exist);
#endif
    ...
    pt_output_stream_destroy_callback cache_output_stream_destroy_callback)
{
    ...
    this->m_cache_input_stream_read_callback = cache_input_stream_read_callback;
    this->m_cache_input_stream_destroy_callback = cache_input_stream_destroy_callback;
    this->m_cache_output_stream_init_callback = cache_output_stream_init_callback;
    this->m_cache_output_stream_write_callback = cache_output_stream_write_callback;
    this->m_cache_output_stream_destroy_callback = cache_output_stream_destroy_callback;
    return true;
}
...
inline bool gfx_connection_vk::load_pipeline_cache(char const *pipeline_cache_file_name, VkPipelineCache *pipeline_cache)
{
    ...
    void *pipeline_cache_data;
    {
        ...
        if (pt_input_stream_ref(-1) != cache_input_stream)
        {
            ...
            {
                if (cache_input_stream_size >= 32)
                {
                    void *cache_input_stream_data = mcrt_aligned_malloc(cache_input_stream_size, alignof(uint8_t));

                    intptr_t res_read = this->m_cache_input_stream_read_callback(cache_input_stream, cache_input_stream_data, cache_input_stream_size);
                    assert(res_read == cache_input_stream_size);

                    uint32_t header_length = *reinterpret_cast<uint32_t *>(reinterpret_cast<uint8_t *>(cache_input_stream_data));
                    uint32_t cache_header_version = *reinterpret_cast<uint32_t *>(reinterpret_cast<uint8_t *>(cache_input_stream_data) + 4);
                    uint32_t vendor_id = *reinterpret_cast<uint32_t *>(reinterpret_cast<uint8_t *>(cache_input_stream_data) + 8);
                    uint32_t device_id = *reinterpret_cast<uint32_t *>(reinterpret_cast<uint8_t *>(cache_input_stream_data) + 12);
                    mcrt_uuid pipeline_cache_uuid = mcrt_uuid_load(reinterpret_cast<uint8_t *>(cache_input_stream_data) + 16);
                    ...
                    {
                        pipeline_cache_size = cache_input_stream_size;
                        pipeline_cache_data = cache_input_stream_data;
                    }
                    else
                    {
                        mcrt_log_print("The header of the assumed vkpipeline cache file %s doesn't match current device!\n", pipeline_cache_file_name);
                        mcrt_aligned_free(cache_input_stream_data);
                ...
                {
                    mcrt_log_print("The size of the assumed vkpipeline cache file %s is less than 32 bytes!\n", pipeline_cache_file_name);
                    pipeline_cache_size = 0U;
                    pipeline_cache_data = NULL;
                }
            }
...
inline void gfx_connection_vk::store_pipeline_cache(char const *pipeline_cache_file_name, VkPipelineCache *pipeline_cache)
{
...

    {
        ...
        if (pt_output_stream_ref(-1) != cache_output_stream)
        {
            ...
            void *pipeline_cache_data = mcrt_aligned_malloc(pipeline_cache_size_before, alignof(uint8_t));
            PT_MAYBE_UNUSED VkResult res_get_pipeline_cache_data = this->m_device.get_pipeline_cache_data(this->m_pipeline_cache_lighting_mesh, &pipeline_cache_size, pipeline_cache_data);
            ...
            intptr_t res_write = this->m_cache_output_stream_write_callback(cache_output_stream, pipeline_cache_data, pipeline_cache_size);
        ...
        {
            mcrt_log_print("The assumed vkpipeline cache file %s can't be accessed!\n", pipeline_cache_file_name);
        }
    }

    this->m_device.destroy_pipeline_cache(this->m_pipeline_cache_lighting_mesh);
    this->m_pipeline_cache_lighting_mesh = VK_NULL_HANDLE;
}
