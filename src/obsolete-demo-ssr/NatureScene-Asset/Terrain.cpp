...
void main()
{
	...
	else
	{
		...
		gl_TessLevelOuter[0] = pow(max(l_Range[0],1.0f),1.25f)*pow(l_Diameter_Projection[0],1.25f)*8.0f; //l_Range����Ϊ1.0����ֹ����ƽ�ĵ��α��޳� //5.0f = 1.0f/2.0f(NormalizedDevice)*800.0f(ScreenSize)/80.0f(TesselationTriangleSize)
...
//Gather�ᵼ�¡�դ�񻯡���ʹ��Linear Sample����Ч���⡰դ�񻯡�����
	...
	if(l_Tessellation_M < -0.2f) 
		l_Slice = 0.0f;
	else if(l_Tessellation_M < 0.2f)					                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   
		l_Slice = 1.0f;
	else
		l_Slice = 2.0f;		

    highp vec3 l_Tessellation_UVW_Albedo = vec3(RS_Tessellation_UV_Height*16.0f, l_Slice); //PerPatch //Sample Wrap MirroredRepeat
