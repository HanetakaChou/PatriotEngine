...
void Scene_Geometry_Grass::Init(const char*szFileName)
{
	...
	assert((pHeader->ddspf.flags & DDS_FOURCC) && (pHeader->ddspf.fourCC == D3DFMT_R32F));

	assert(pHeader->width == 512U);

	float (*pTextureData)[512] = reinterpret_cast<float (*)[512]>(reinterpret_cast<uintptr_t>(pFileData) + sizeof(uint32_t) + sizeof(DDS_HEADER));
	...
	for (int i_x = 0; i_x < 16; ++i_x)
	{
		for (int i_y = 0; i_y < 16; ++i_y)
		{
			...
			for (int i_Grass = 0; i_Grass < Num_Grass; ++i_Grass)
			{
				...
				int l_V = 512 - (16 * i_y + l_Y_16) * 2;
				int l_U = (16 * i_x + l_X_16) * 2;
