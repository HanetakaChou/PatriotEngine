...
细分标准（Subdivision Criterion）      
1.比较 相邻元 的函数值 的差值 //细分 超出阈值 的元 //即细分变化较大的元  
渲染时 使用 线性元 //问题1： 过度细分 值变化较大 但变化率为线性 的元 （线性元能很好的近似，并不用细分） //问题2： 无法检测到 值变化较小 但是有局部极值的元  //问题原因在于 节点值 并没有提供 足够的关于函数在 元内部 的行为的信息
